...
class Glassdoor(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        if location_type is None:
            if scraper_input.is_remote:
                log.warning(
                    "Glassdoor: location lookup failed; falling back to default remote location."
                )
    ...
    def _fetch_jobs_page(
        ...
        try:
            ...
            if "errors" in res_json:
                ...
                # Some responses include non-fatal errors but still return data.
                jobs_data_fallback = (
                    res_json.get("data", {})
                    .get("jobListings", {})
                    .get("jobListings", [])
                )
                if not jobs_data_fallback:
                    raise ValueError(f"Error encountered in API response: {err_msg}")
                log.warning(f"Glassdoor GraphQL warning: {err_msg}")
        except (
    ...
    def _fetch_job_description(self, job_id):
        ...
        try:
            # Use scraper session + timeout to avoid hanging worker threads.
            res = self.session.post(url, data=json.dumps(body), timeout_seconds=10)
        except Exception:
            return None
    ...
    def _get_location(self, location: str, is_remote: bool) -> (int, str):
        # For remote search, keep location scoping when provided.
        # The old hardcoded location id was US-specific and caused errors on non-US domains.
        ...
        if res.status_code != 200:
            if res.status_code == 429:
                ...
                if is_remote:
                    log.warning(f"{err}. Falling back to default remote location.")
                    return "11047", "STATE"
                log.error(err)
                return None, None
            else:
                ...
                if is_remote:
                    log.warning(
                        f"Glassdoor location lookup failed ({res.status_code}); "
                        "falling back to default remote location."
                    )
                    return "11047", "STATE"
