...
class BDJobs(Scraper):
    # Canonical job search (jobs.bdjobs.com now 302s here; following redirects reliably avoids 302 "errors").
    base_url = "https://bdjobs.com"
    search_url = "https://bdjobs.com/h/jobs/"
    delay = 2
    band_delay = 3
    ...
    def __init__(
        ...
        self.session = create_session(
            ...
            has_retry=True,
            delay=5,
            # Clearing cookies on every request breaks some redirect / session flows.
            clear_cookies=False,
        )
    ...
    def _get_follow(self, url: str, *, params: dict | None = None, timeout: int = 60):
        """
        GET with redirects enabled; if the client still ends on 3xx, follow Location manually.
        """
        ...
        max_hops = 10
        while response.status_code in (301, 302, 303, 307, 308) and max_hops > 0:
            loc = response.headers.get("Location")
            if not loc:
                break
            next_url = urljoin(response.url, loc)
            response = self.session.get(next_url, timeout=timeout, allow_redirects=True)
            max_hops -= 1
        return response
            ...
            try:
                ...
                if response.status_code != 200:
                    log.error(
                        ...
                        f"(final url: {getattr(response, 'url', '')})"
                    )
                    break
    ...
    def _get_job_details(self, job_url: str) -> Dict[str, Any]:
        ...
        """
        try:
            response = self._get_follow(job_url, timeout=60)
            if response.status_code != 200:
                return {}
