        ...
        while continue_search():
            ...
            try:
                ...
                if response.status_code not in range(200, 400):
                    ...
                    if (
                        ...
                        or "recaptcha" in lowered_body
                        or "security check" in lowered_body
                    ):
                        log.warning(
                            "Naukri request blocked by CAPTCHA/security challenge. "
                            "Stopping Naukri scrape and returning partial results."
                        )
                ...
                if isinstance(data, dict):
                    data_blob = str(data).lower()
                    if (
                        "captcha" in data_blob
                        or "recaptcha" in data_blob
                        or "security check" in data_blob
                    ):
                        log.warning(
                            "Naukri API response indicates CAPTCHA/security challenge. "
                            "Stopping Naukri scrape and returning partial results."
                        )
                        return JobResponse(jobs=job_list)
                ...
                if (
                    ...
                    or "security check" in err_text
                ):
                    log.warning(
                        "Naukri request failed due to CAPTCHA/security challenge. "
            ...
            for job in job_details:
                ...
                try:
                    ...
                    # Keep scraping when one listing has malformed payload.
                    continue
