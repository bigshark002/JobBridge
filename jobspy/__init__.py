...
def scrape_jobs(
    ...
    **kwargs,
    ...
    scraper_input = ScraperInput(
        ...
        offset=offset,
        hours_old=hours_old,
        wellfound_company_name=wellfound_company_name,
        wellfound_pages_to_fetch=wellfound_pages_to_fetch,
    ...
    for site, job_response in site_to_jobs_dict.items():
        for job in job_response.jobs:
            ...
            _jt = job_data["job_type"]
            if not _jt:
                job_data["job_type"] = None
            elif isinstance(_jt, list):
                job_data["job_type"] = ", ".join(
                    jt.value[0] for jt in _jt
                )
            else:
                job_data["job_type"] = str(_jt)
            job_data["emails"] = (
